...
Updated: 2026-08-27 · dataset version/provenance lifecycle evidence applied

| ID | Question / 问题 | Status | Next evidence |
|---|---|---|---|
| C-W01 | How are GB/Z 185.1–185.7 implemented by platforms and industries? | OPEN | Adoption and conformance evidence |
| C-W02 | How do GB/Z 185, ASL, MCP and A2A map across identity, discovery, interaction, tools and authorization? | OPEN | Formal mappings |
| C-W03 | What becomes normative in the agent-model governance framework? | OPEN | Draft and consultation |
...
| C-W33 | Can quality evaluation preserve provenance across real-machine, simulation, training-base and interaction feedback data? | **STRENGTHENED / open** | Version/provenance work item plus evaluation-system schemas |
...
| C-W39 | How are standard version, tool version, dataset version and model/application context preserved in reusable results? | **STRONGLY STRENGTHENED / W35 PRIORITY** | Dedicated dataset version/provenance work item; need schema text |
...
| C-W50 | Does a trusted-data-space usage-control policy preserve dataset version, quality-result version and model/task scope? | **STRONGLY STRENGTHENED / W35 PRIORITY** | Version/provenance direction exists; need policy-template integration |
...
| C-W61 | Which additional public cases show application feedback changing dataset/model state with traceable evidence? | **STRONGLY STRENGTHENED / monitor** | New lifecycle/version cases; seek feedback-linked version diffs |
| C-W62 | Does a platform-level `回流` function expose a machine-readable feedback-event schema rather than generic data movement? | **NEW / W35 PRIORITY** | Platform APIs, event schemas and sample records |
| C-W63 | Can returned feedback be linked to before/after dataset versions and the exact action that changed them? | **STRONGLY STRENGTHENED / W35 PRIORITY** | Version-management work item + case implementations; need exact task linkage |
| C-W64 | Do automatic-return pipelines enforce explicit quality/governance admission before returned data enters training or evaluation sets? | **NEW / W35 PRIORITY** | Production gating rules, rejection logs and audit evidence |
| C-W65 | How does one model/Agent evaluation diagnosis become a concrete, owned and auditable dataset-optimization task? | **NEW / W35 PRIORITY** | 模数共振 implementations, task schemas and workflow evidence |
| C-W66 | Can evaluation datasets remain diagnostically independent while their results drive new training-data construction? | **NEW / W35 PRIORITY** | Evaluation governance, contamination controls and implementation guidance |
| C-W67 | Can before/after dataset versions and later evaluation results prove whether targeted optimization actually closed the diagnosed capability gap? | **STRONGLY STRENGTHENED / W35 PRIORITY** | Version spine now explicit; need causal task/result linkage |
| C-W68 | What exact metadata and identifier model will `高质量数据集 版本管理与溯源技术要求` define? | **NEW / W35 PRIORITY** | Draft/publication text, schemas and validation pilots |
| C-W69 | Can one evaluation diagnosis reference the exact prior dataset version and create a data-work-item that points to the resulting version? | **NEW / W35 PRIORITY** | Workflow schemas, task IDs and before/after lineage records |
| C-W70 | Can dataset version lineage preserve quality status, rights/compliance status and source restrictions without conflating them? | **NEW / W35 PRIORITY** | Version metadata profiles and platform examples |
| C-W71 | Does every new feedback-driven dataset version trigger mandatory quality/governance re-evaluation before training admission? | **NEW / W35 PRIORITY** | Lifecycle rules, gates and audit evidence |
